			...
			public static function get_data_penggajian()
				{
					...
					$list_penggajian = $CI->M_penggajian->get_datatables_karyawan($where);
					$data = array();
					$no = $_POST['start'];
					...
					foreach($list_penggajian as $field)
						{
							...
							$row[] 	= $field->nama_jabatan;
							$row[] 	= $field->nama_divisi;
							$row[]	= "<a target='_blank' href='".site_url('slip_gaji/cetak/'.$field->id_karyawan)."' class='btn btn-primary' title='Cetak Slip Gaji'><i class='fa fa-print'></i></a> ";
                            $data[] = $row;
						}

					$output = array(
						'draw'				=> $_POST['draw'],
						'recordsTotal'      => $CI->M_penggajian->count_all_karyawan(),
	                    'recordsFiltered'   => $CI->M_penggajian->count_filtered_karyawan($where),
	                    'data'              => $data,
					);
			...
			public static function tambah_penggajian_karyawan()
				{
					...
					if(!empty($tgl_awal) && $tgl_awal != null && !empty($tgl_akhir) && $tgl_akhir != null)
							...
							if($time1 > $time2)
							...
							else
								{
									...
									$jumlah_data_penggajian = $CI->M_penggajian->tampil_gaji_karyawan(
											"(tgl_awal BETWEEN '".$tanggal_awal."' AND '".$tanggal_akhir."') AND(tgl_akhir BETWEEN '".$tanggal_awal."' AND '".$tanggal_akhir."')"
										)->order_by('tgl_akhir','desc')->count_all_results();

									if($jumlah_data_penggajian != 0)
										{
											$response['status'] = 'sudah ada';
										}
									else
										{
													...
													if(!in_array($abs->id_karyawan, $data_gaji_kar))
														{
															...
															else if($metode_pembayaran == 'perbulan')
																{
									...
									# lakukan pengecekan absen detail per karyawan
									// foreach($data_pegawai as $dp =>$value)
									// 	{
									// 		$tampil_absensi_karyawan = $CI->M_penggajian->tampil_absen_gaji(array(
									// 						'id_karyawan'	=> $value,
									// 						'tgl_absen >=' 	=>$tanggal_awal,
									// 						'tgl_absen <='	=> $tanggal_akhir,
									// 					));

									// 		$tgl_awal_penggajian = $CI->M_penggajian->tampil_gaji_karyawan(
									// 			"id_karyawan='".$value."' AND (tgl_awal BETWEEN '".$tanggal_awal."' AND '".$tanggal_akhir."') AND(tgl_akhir BETWEEN '".$tanggal_awal."' AND '".$tanggal_akhir."')"
									// 		)->order_by('tgl_awal','asc')->limit(1)->get()->result();

									// 		foreach($tgl_awal_penggajian as $tawal)
									// 			{
									// 				$tgl_pertama = $tawal->tgl_awal;
									// 			}
									...
									// 		if(@$tgl_pertama != null && @$tgl_terakhir != null)
									// 			{
									// 				$list_tanggal1 = $CI->list_tanggal($tgl_pertama,$tgl_terakhir);
													
									// 				$data_pegawai['karyawan-'.$value] = $list_tanggal1;
													
									// 				// unset($data_pegawai['karyawan-'.$value]);
						...
						{
							$response['status'] = 'tanggal kosong';
						}
